...
impl Parcelable for () {
    fn deserialize(_parcel: &mut Parcel) -> BinderResult<Self>
    where
        Self: Sized,
    {
        Ok(())
    ...
    fn serialize(&self, _parcel: &mut Parcel) -> BinderResult<()> {
        Ok(())
    }
}
...
impl<K: Parcelable + Eq + std::hash::Hash, V: Parcelable> Parcelable for HashMap<K, V> {
    fn deserialize(parcel: &mut Parcel) -> BinderResult<Self> {
